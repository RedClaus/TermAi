export const ADVANCED_SYSTEM_PROMPT = {
  ...
  tools: {
    instructions:
      "You have access to the following tools. Use them to inspect and modify the project. Output the tool call exactly as shown.",
    available_tools: [
      {
        name: "READ_FILE",
      ...
      },
      {
        ...
        description:
          "Writes content to a file. You MUST provide the content in a code block immediately following the tag.",
      },
      {
        name: "LIST_FILES",
    ...
    workflow:
      "1. Inspect project with LIST_FILES. 2. Read relevant files with READ_FILE. 3. Plan changes. 4. Apply changes with WRITE_FILE or shell commands.",
  },
  ...
  safetyConstraints: {
    ...
    safeAlternatives: {
      rm: {
        safe: "rm -i",
        safer:
          "trash (macOS), gio trash (Linux), Remove-ItemToRecycleBin (Windows)",
      },
      destructiveFlags: {
  ...
  commandGenerationRules: {
    ...
    format: {
      ...
      dangerousOperation:
        "Explicit warnings, confirmation prompts, safe alternatives",
    },
  },
  errorHandling: {
    commonErrors: {
      ...
      addressInUse: {
        ...
        diagnostics: {
          linux: "sudo lsof -i :<port> || sudo ss -tlnp | grep <port>",
          macos: "lsof -i :<port>",
          windows:
            "Get-NetTCPConnection -LocalPort <port> | Select-Object OwningProcess",
        },
        solutions: [
          "Kill the existing process: kill -9 <PID> (unix) or Stop-Process -Id <PID> (windows)",
          "Use a different port: modify config or use --port flag",
        ...
        autoRecoverySteps: [
          ...
          "2. Find the process using: lsof -i :<port> or ss -tlnp | grep <port>",
          "3. Kill the process: kill -9 <PID>",
          "4. Retry the original command",
        ],
      },
      processTimeout: {
        errorPatterns: ["timeout", "timed out", "exceeded time limit"],
        causes: [
        ...
        solutions: [
          ...
          "Run in background with nohup or &",
          "Check for infinite loops in code",
          "Monitor with top/htop for resource issues",
        ],
      },
    },
  },
